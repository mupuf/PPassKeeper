...
class ListPwd
{
	private:
		...
		static void appendEntries(const char* dir, unsigned int entry_types, unsigned int flags, std::vector<ppk_entry*>& list);
	public:
		static unsigned int getEntryListCount(const char* dir, unsigned int entry_types, unsigned int flags);
		static ppk_entry** getEntryList(const char* dir, unsigned int entry_types, unsigned int* nbEntries, unsigned int flags);
};
